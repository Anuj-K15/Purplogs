...
const Navbar = () => {
  ...
  const logoutHandler = async (e) => {
    try {
      ...
      console.log(error);
      // Safe error handling in case response is undefined
      toast.error(
        error.response?.data?.message || "Logout failed. Please try again."
      );

      // If there's a connection issue, still log the user out locally
      if (error.message === "Network Error") {
        navigate("/");
        dispatch(setUser(null));
        toast.warning(
          "You've been logged out locally, but we couldn't reach the server."
        );
      }
    }
  };
  ...
  return (
    <div className="py-2 fixed w-full bg-white/80 backdrop-blur-sm dark:bg-gray-900/90 dark:border-b-purple-800/30 border-b-purple-300 border-b z-50 shadow-sm transition-colors duration-300">
        ...
        <div className="flex gap-7 items-center">
          <Link to={"/"}>
            <div className="flex gap-2 items-center">
              ...
              <h1 className="font-bold text-3xl md:text-4xl bg-gradient-to-r from-purple-600 to-blue-500 bg-clip-text text-transparent">
                Purplogs
              </h1>
            </div>
          </Link>
          <div className="relative hidden md:block">
            <Input
              type="text"
              placeholder="Search"
              className="border border-purple-200 dark:border-purple-900 dark:bg-gray-900 bg-white w-[300px] hidden md:block focus:border-purple-500 focus:ring-purple-500"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <Button
              className="absolute right-0 top-0 bg-purple-600 hover:bg-purple-700 dark:bg-purple-700 dark:hover:bg-purple-800"
