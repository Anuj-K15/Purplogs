...
const API_IMPORT = "import api from '../lib/api';";
...
async function processFile(filePath) {
  try {
    ...
    // Check if file already has hardcoded URLs
    if (content.includes(OLD_API_URL)) {
      ...
      if (
        ...
        content = content.replace(AXIOS_IMPORT, `${API_IMPORT}`);
      }
      ...
      // since it's already set in the api.js config
      content = content.replace(
